...
async function loadSavedParkings() {
    try {
        ...
        const response = await fetch(
            "api/SavedParkings/index.php?action=list&user_id=" +
                user.id_utilisateur,
        );
...
async function addSavedParking(parkingData, customName, note) {
    try {
        ...
        const response = await fetch(
            "api/SavedParkings/index.php?action=save&user_id=" +
                user.id_utilisateur,
            {
                method: "POST",
        ...
        if (result.success) {
            ...
            // mettre à jour l'étoile dans le popup si nécessaire
            try {
                updatePopupFavorite(parkingData.id, true);
            } catch (e) {}
...
// Mettre à jour l'étoile dans le popup ouvert si présent
function updatePopupFavorite(parkingId, saved) {
    try {
        const buttons = document.querySelectorAll(".parking-popup-favorite");
        buttons.forEach((btn) => {
        ...
        console.warn("updatePopupFavorite failed", e);
    }
}

// Supprimer un parking
async function deleteSavedParking(parkingId) {
    const confirm_msg = typeof t === "function"
        ? t("save_parking_delete_confirm")
        : "Êtes-vous sûr de vouloir supprimer ce parking ?";
    ...
    try {
        ...
        const response = await fetch(
            "api/SavedParkings/index.php?action=delete&user_id=" +
                user.id_utilisateur,
            {
                method: "POST",
                headers: { "Content-Type": "application/json" },
        ...
        if (result.success) {
            ...
            try {
                updatePopupFavorite(parkingId, false);
            } catch (e) {}
        ...
        item.innerHTML = `
            <div class="vehicle-item-info" onclick="goToSavedParking('${parking.id}')">
                ...
                <div class="vehicle-item-details">
                    ${
            parking.note
                ? `<div style="font-size: 12px; color: #666;">${parking.note}</div>`
                : ""
        }
                </div>
            </div>
...
function initSaveParkingModal() {
    ...
    if (deleteBtn) {
        deleteBtn.onclick = async () => {
            if (window.currentEditingParking) {
                const success = await deleteSavedParking(
                    window.currentEditingParking.id,
                );
                if (success) {
                    closeSaveParkingModal();
                    const msg = typeof t === "function"
                        ? t("save_parking_success")
                        : "Parking supprimé !";
    ...
    if (form) {
        form.onsubmit = async (e) => {
            ...
            const name = document.getElementById("parkingCustomName").value
                .trim();
            const note = document.getElementById("parkingNote").value.trim();

            if (!name) {
                document.getElementById("saveParkingError").textContent =
                    "Le nom est requis";
                document.getElementById("saveParkingError").style.display =
                    "block";
                return;
            }

            const success = await addSavedParking(
                window.currentEditingParking,
                name,
                note,
            );
            if (success) {
